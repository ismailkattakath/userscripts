...
- the label reads **Sending**, then **Stored** or **Failed**, then settles back to **Store**
...
- **Downloads land wherever MeTube puts them**, at `best` quality in `mp4`. This script has no opinion about that and no UI for it — MeTube's own page does.
